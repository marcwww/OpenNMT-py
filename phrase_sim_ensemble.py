...
def save_checkpoint(models, epoch,
                    losses, accurs,
                    precs, recalls,
                    f1s, name):
    progress_clean()
    ...
    for model_idx in range(len(models)):
        ...
        model_fname = basename + ".model%d" % (model_idx)
...
def train(train_iters, val_iter, epoch, models,
    ...
    for epoch in range(epoch_start,epoch_end):
        ...
        if (epoch+1) % SAVE_PER == 0:
            save_checkpoint(models,epoch,losses,accurs,precs,recalls,f1s,
                            opt.exp)

def valid(val_iter, models):
    for model_idx in range(len(models)):
        model = models[model_idx]
        model.eval()
...
if __name__ == '__main__':
    ...
    for i in range(4):
        ...
        if opt.load_idx != -1:
            basename = "{}-epoch-{}".format(opt.exp, opt.load_idx)
            model_fname = basename + ".model%d" % (i)
