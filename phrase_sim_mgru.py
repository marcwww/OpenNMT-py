...
class PhraseSim(nn.Module):
    ...
    def forward(self, seq1, seq2):
        ...
        _, gru_h1 = self.gru_encoder(seq1)
        _, gru_h2 = self.gru_encoder(seq2)

        pooling_h1 = self.pooling_encoder(seq1)
        pooling_h2 = self.pooling_encoder(seq2)

        h1 = torch.cat([gru_h1, pooling_h1], dim=-1)
        h2 = torch.cat([gru_h2, pooling_h2], dim=-1)
...
class PoolingEncoder(nn.Module):
    ...
    def __init__(self, embedding, hdim, dropout=0.5):
...
if __name__ == '__main__':
    ...
    gru_encoder = GruEncoder(embedding,
                      ...
                      opt.dropout,
                      opt.bidirection)
    pooling_encoder = PoolingEncoder(embedding,
                                     opt.rnn_size,
                                     opt.dropout)

    model = PhraseSim(gru_encoder, pooling_encoder, opt.dropout).to(device)
