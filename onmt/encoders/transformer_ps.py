...
class TransformerEncoder(EncoderBase):
    ...
    def forward(self, src1, src2):
        ...
        # emb : (seq_len, bsz, hdim)
        emb1 = self.embeddings(src1)
        emb2 = self.embeddings(src2)
        emb2 += self.src2_bias

        seq_len, bsz, hidm = emb1.shape
        emb = torch.cat([emb1,emb2],dim=0).view(-1, bsz, hidm)

        src = torch.cat([src1,src2],dim=0).view(-1, bsz, 1)
        ...
        return Variable(emb.data), out.transpose(0, 1).contiguous(), src
